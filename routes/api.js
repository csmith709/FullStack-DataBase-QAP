...
router.post('/clients', async (req, res) => {
    ...
    try {
        await db.query('INSERT INTO Client (first_name, last_name, email, phone_number) VALUES ($1, $2, $3, $4)', [first_name, last_name, email, phone_number]);
...
router.delete('/clients/delete/:id', async (req, res) => {
    ...
    try {
        ...
        res.redirect('/update-success?operation=delete&type=client&id=' + id);
    } catch (err) {
        res.status(500).send(err.message);
    }
});
...
router.put('/employees/:id', async (req, res) => {
    ...
    try {
        const updateQuery = `
            ...
            WHERE employee_id = $5
        `;
        await db.query(updateQuery, [first_name, last_name, phone_number, email, id]);
